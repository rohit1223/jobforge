...
4. **Stop and present the report.** Phase 1 may be the whole job.
...
6. **Write `resume-tailored.tex`** = copy of master + approved edits. **Content only** — never touch the preamble, packages, fonts, or layout.
7. **Ensure toolchain, then compile:** run `scripts/ensure-toolchain.sh` then `scripts/compile-resume.sh <dir>/resume-tailored.tex`. See **Toolchain** below. The compile script auto-installs missing LaTeX packages, reverts to no PDF on error, and **warns if the PDF exceeds 1 page**.
8. **Report** the final score, the diff summary, the PDF path, and any `UNVERIFIED`/`QUANTIFY` markers the user must still resolve.
